...
class FocalLoss(_WeightedLoss):
    ...
    def softmax_loss(self, input, target):
        ...
        alphas = target.data.gt(0).float() * self.alpha + \
                 target.data.eq(0).float() * (1 - self.alpha)
        ...
        if self.size_average:
            try:
                ...
                print(target.size(), 'target labels: [{:d}, {:d}]'.format(target.min().item(), target.max.item()))
                print(fl.max(), fl.min())
    ...
    def sigmoid_loss(self, input, target):
        ...
        fg_nums = target.data.gt(0).sum().item()
        n, c = input.size(0), input.size(1)
        t = to_one_hot(target.data.cpu(), c + 1)
        t = t[:, 1:]
        t = t.contiguous().cuda()
    ...
    def forward(self, input, target):
        if self.activation == 'sigmoid':
            ...
            raise ValueError('Unknown activation function: {:s}'.format(self.activation))
